function mirrorDrawTool() {
	...
	this.populateOptions = function () {
		...
		self.verticalOption = createSpan(`<img src='assets/vertical.png' id='vertical' class='mirrorOptions'>`);

		self.horizontalOption = createSpan(`<img src='assets/horizontal.png' id='horizontal' class='mirrorOptions'>`);

		select(".options").child(self.verticalOption);
		select(".options").child(self.horizontalOption);
		self.horizontalOption.style('opacity', '0.2');

		select("#horizontal").mouseClicked(function () {
			if (self.axis == "x") {
				self.axis = "y";
				self.lineOfSymmetry = height / 2;
				self.horizontalOption.style('opacity', '1');
				self.verticalOption.style('opacity', '0.2');
			}
		});

		select("#vertical").mouseClicked(function () {
			if (self.axis == "y") {
				self.axis = "x";
				self.lineOfSymmetry = width / 2;
				self.verticalOption.style('opacity', '1');
				self.horizontalOption.style('opacity', '0.2');
			}
		});
	};
